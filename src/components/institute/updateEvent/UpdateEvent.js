...
import { getInstituteEventById, updateInstituteEvent } from "../../../actions/instituteEvent/instituteEvent";
...
const UpdateEvent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const result = await dispatch(getInstituteEventById(id));
  ...
  const onFinish = async () => {
    try {
      ...
      const response = await dispatch(updateInstituteEvent(formData));
      if (response.success) {
